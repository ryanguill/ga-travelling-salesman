...
public class Main {
	...
	public static void main(String[] args) {
		...
		if (doesPropertiesFileExist(settingsDirectory)) {
			...
			System.out
					.println("Properties file not found / Creating default properties file: ");
			System.out.println();
		...
		GeneticAlgorithmTravellingSalesman ga = new GeneticAlgorithmTravellingSalesman(
				config);
	...
	private static boolean doesPropertiesFileExist(File settingsDirectory) {
		...
		File propFile = new File(settingsDirectory.toString() + "/"
				+ propertiesFileName);
	...
	private static Properties readProperties(File settingsDirectory) {
		...
		try {
			prop.load(new FileInputStream(settingsDirectory.toString() + "/"
					+ propertiesFileName));
		} catch (IOException e) {
