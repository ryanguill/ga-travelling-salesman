...
public class GeneticAlgorithmTravellingSalesman {
	...
	/*
	private static final int NUMBER_OF_CHROMOSOMES_PER_GENERATION = 5000; //population size
	private static final int NUMBER_OF_GENERATIONS_TO_RUN = 250000;
	private static final int DUPLICATE_LOCATION_SCORE_PENALTY = 5000;
	private static final int CHROMOSOME_LENGTH_MULTIPLIER = 10;
	private static final double CHROMOSOME_SHUFFLE_ODDS_MULTIPLIER = 2.0D;
	private static final int SUPER_INDIVIDUAL_BONUS_FIRST = 10000;
	private static final int SUPER_INDIVIDUAL_BONUS_SECOND = 5000;
	private static final int SUPER_INDIVIDUAL_BONUS_THIRD = 1000;
	private static final double FIRST_MUTATION_PERCENTAGE = 25.0D; //10.0
	private static final double SECOND_MUTATION_PERCENTAGE = 17.0D; //7.0
	private static final double THIRD_MUTATION_PERCENTAGE = 10.0D;  //5.0
	//private static final int NUMBER_OF_GENERATIONS_TO_SHOW = 500;
	 */
	...
	public void commenceAlgorithm ( ) {
		...
		while ( currentGeneration < config.getNUMBER_OF_GENERATIONS_TO_RUN() && isRunning )
		{
			...
			out("Gen's time: " + nf.format((double) generationTime / 1000000) + "ms. / Avg Gen Time: " + nf.format(averageGenTimeMS) + "ms. / Total Time: " + nf.format((double) totalTime / 1000000000) + " s.");
		}		
		...
		out("best solution found: " + nf.format(bestIndividual.getLocationsFound()) + " locations / " + nf.format(bestIndividual.getTotalDistance()) + " Total Distance / " + nf.format(bestIndividual.getFitnessScore()) + " fitness score. / Gen Found: " + bestIndividualGeneration);
		out(solution);
	...
	public void runGeneration ( ) {
		...
		out("---------------------------------------------------------------");
		out("Gen: " + currentGeneration + "/" + config.getNUMBER_OF_GENERATIONS_TO_RUN() + " - " + nf.format( ((double) currentGeneration / config.getNUMBER_OF_GENERATIONS_TO_RUN()) * 100 ) + "%" + " - Est. Time Remaining: " + nf.format(((config.getNUMBER_OF_GENERATIONS_TO_RUN() - currentGeneration) * averageGenTimeMS) / 1000) + "s." );
		...
		out("Gen's Avg Score: " + nf.format(currentAverageScore) + " / Best Avg Score: " + nf.format(bestAverageScore) + " / Best Score: " + nf.format(bestIndividual.getFitnessScore()));
	...
	public Chromosome scoreChromosomeFitness ( Chromosome c ) {
		...
		//find the genes in teh chromosome that are actual locations
		...
		if ( actualLocationsLength > 1 )
		{
			...
			for ( int alIndex = 0; alIndex < actualLocationsLength; alIndex++ ) {
				...
				double dist = getLatLongDistance(currentLocation.getLattitude(), currentLocation.getLongitude(), previousLocation.getLattitude(), previousLocation.getLongitude());
		...
		if ( skyFound ) {
			System.out.println("SKYNET found in chromosome! It should be selected out - but you might want to stop to be sure.");
		}		
	...
	public ArrayList<Chromosome> mateTopChromosomes ( ArrayList<Chromosome> chromosomesForMating) {
		...
		//we are trying to create a new population of NUMBER_OF_CHROMOSOMES_PER_GENERATION
		
		//distribute the chromosomes for mating evenly determined by score
		//this should effectively make the stronger individuals more likely to be picked

		ArrayList<Chromosome> distributedChromosomesForMating = new ArrayList<Chromosome>();
		
		//first we need to get the total score of the chromosomes selected for mating
		double totalScore = 0;
		Chromosome c;
		int i;	
		...
		while ( output.size() < config.getNUMBER_OF_CHROMOSOMES_PER_GENERATION() )
		{
			...
			//pick a random number between 0 and length of chromosomesForMating - 1 for the first individual
			int randomPick1 = randRange(0, chromosomesForMatingLength - 1);
			
			//now pick another random number between 0 and length of chromosomesForMating - 1 for the second individual
